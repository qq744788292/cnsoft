...
import org.jfpc.base.cache.SessionHelper;
...
public class MyControllerSupport extends MyFrameworkSupport implements ISFrameworkConstants {
	...
	public boolean doCheckLogin(HttpServletRequest request, HttpServletResponse response) {
		if (logger.isDebugEnabled())
			logger.error("requestUrl//////////>>>>>>=====" + request.getRequestURI());
		// 定义sessionID
		...
		if (StringUtils.isEmpty(token)) {
			// 获得TOKEN数据
			if (StringUtils.isEmpty(super.getToken())) {
			// 从缓存里面获得
...
//				}// 用URL里面获得
		...
		if (logger.isDebugEnabled())
			logger.error("Token///////////////>>>>>>=====" + super.getToken());
	...
	private void goBack(HttpServletRequest request, HttpServletResponse response){
		...
		if (logger.isDebugEnabled())
			logger.error("doCheckLogin>>>>>>=====" + _callBackUrl);
		try {
			if (StringUtils.isEmpty(_callBackUrl))
