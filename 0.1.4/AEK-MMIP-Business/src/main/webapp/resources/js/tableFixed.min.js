(function($) {
	$.fn.jFixedtable = function(options) {
		...
		return this.each(function() {
			...
			$table = $(this);
			$(this).attr("id", "");
			this.sBase = $("<div style=\"width:100%\"></div>");
            ...
            options.height = options.height == 0 ? $table.parents().height() - 2 : options.height;
            var width = ($table.parents().width() || options.width);
			this.ToFixTb = function() {
				...
				if (this.headerRows > 0) {
					for (var i = 0; i < this.headerRows; i++) {
						this.sHeaderHeight += parseInt($table.find("tr").eq(options.headerRows - 1).outerHeight(),10);
					};
				...
				if(options.headDispay) this.sBase.append(this.sHeader);
				if(options.headDispay) this.sHeader.append(this.sHeaderInner);
				if (this.fixedCols > 0) {
					this.sFData.append(this.sFDataInner);
					this.sBase.append(this.sFData);
				};
				if(options.headDispay) this.sData.append(this.sDataTable);
				if(options.headDispay) this.sBase.append(this.sData);
				if (this.fixedCols > 0) {
					this.sFHeaderWidth = typeof this.sDataTable[0].tBodies[0].rows[this.headerRows] != "undefined" ? this.sDataTable[0].tBodies[0].rows[this.headerRows].cells[this.fixedCols].offsetLeft + 1 : 0;
				} else this.sFHeaderWidth = 0;
				this.sFHeader.css("width", this.sFHeaderWidth + "px");
                this.sHeaderTable.css({
                    "min-width":"100%",
                    "width":"100%"
                });
                var max_width = this.sHeader.width();
                    max_width = max_width > width ? width : max_width;
                this.sHeaderHeight = this.sHeaderHeight - 4 || 36;
				if(options.headDispay){
					this.sData.css({
					...
					this.sDataTable.css({
						"margin-left": -this.sFHeaderWidth,
						"margin-top": -this.sHeaderHeight - 5,
	                    "min-width":"100%"
					});
				}
						...
						if (j >= parseInt(op.ftb.fixedCols)) {
							...
							if (edittype) {
								$(this).bind("click", function(event, itd, ieditpye, ith) {
									if (itd) editcell(itd, ieditpye, ith);
									else editcell($(this), edittype, ths[j]);
								});
							}
						}
					} else {
					...
					if (op.ftb) {
						if (op.ftb.fixedCols > 1) {
							...
							hebox.click(function() {
								gettrs().each(function() {
									...
									if (cbox) {
										if (hebox.is(":checked")) cbox.attr("checked", "true");
										else cbox.attr("checked", "");
									}
								});
								getsfdatatr().each(function() {
									var sfcbox = $(this).find("td:first").find(":checkbox");
									if (sfcbox) {
										if (hebox.is(":checked")) sfcbox.attr("checked", "true");
										else sfcbox.attr("checked", "");
									}
								});
							});
						} else {
						...
						rlchbox.click(function() {
							gettrs().each(function() {
								var cbox = $(this).find("td:first").find(":checkbox");
								if (cbox) {
									if (rlchbox.is(":checked")) cbox.attr("checked", "true");
									else cbox.attr("checked", "");
								}
							});
						});
