...
public class FilePathHelper implements ISFrameworkConstants {
	...
	private static String CONTENT_TYPE_FILE = "/fileContentType.xml";
	...
	static {
		...
		{ ".png", ".1", "image/png","0" }, 
		{ ".gif", ".2", "image/gif","0" }, 
		{ ".bmp", ".3", "application/x-bmp","0" }, 
		{ ".jpeg", ".4", "image/jpeg","0" }, 
		{ ".html", ".5", "text/html","1" },
		{ ".pdf", ".6", "application/pdf","1" }, 
		{ ".htm", ".7", "text/html","1" },
		{ ".doc", ".8", "application/msword","1" } 
		};
			...
			for(String[] item :IMAGE){
				...
				is.add(item[0]);is.add(item[1]);is.add(item[2]);
...
//			InputStream in = FilePathHelper.class.getResourceAsStream(CONTENT_TYPE_FILE);
//			props.load(in);
