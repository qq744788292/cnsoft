...
function doSubmit(){
	if(document.all("actionURL").value==""||document.all("jsonData").value==""){
		alert("请输入业务测试内容");
		return;
	}
	TestForm.submit();
...
function doChange(){
	document.all("actionURL").value="/APITest/"+document.all("hosId").value+document.all("operationId").value+"/"+document.all("hosId").value+"/"+document.all("bizName").value ;
}
</script>
		<form id="TestForm" action="/APITest" method="post">
		 ...
		 HOS_ID:<br><input type="text" id = "hosId" name="hosId" size="100" value="HEMED"/><br>
		 operationId:<br><input type="text" id = "operationId" name="operationId" size="100" value="123456"/><br>		 
		 ...
		 <br>
		 Json Param:<br><textarea id="jsonData" name="jsonData" rows="20" cols="100">${jsonData}</textarea>
		</form>
