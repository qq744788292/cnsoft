package com.hundsun.med.hdp.service.sync.doctor;
 ...
 * @see <DoctorSendThread>
 */
public class DoctorDataSyncService extends AHospitalDataSyncService {
	private static final Logger logger = LoggerFactory.getLogger(DoctorDataSyncService.class);
	...
	public boolean doProcessOnce(String hosId) throws Exception {
		...
		if (doInit() && doCheck() && doProcess() && doSend()) {
			logger.debug("门诊科室下医生数据同步结束.....");
		} else {
			logger.debug("门诊科室下医生数据同步失败xxxxx");
		}
