...
import com.hundsun.med.hds.service.sync.AHospitalDataAccessService;
...
public class HospitalDataSynchronizationAPI extends MyCloundAPISupport {
	private Logger logger = LoggerFactory.getLogger(HospitalDataSynchronizationAPI.class);
	...
	public RESTResultBean doReceive(HttpServletRequest request, @PathVariable String hosId, @PathVariable String bizName, @PathVariable String encryType, String jsonData, String doNow) {
		...
		if (ONE.equals(rs.getCode())) {
			...
			mqService.offerObjectInList(HospitalJobKeyService.getSyncDataKeyName(hosId, bizName), cloundSecyrityConfig.decryption(HospitalInfoService.getHosSyncConfig(hosId), (String) rs.getResult(), encryptionType));
		}
		...
		if (ONE.equals(doNow)) {
			try {
				...
				ahds.setHosId(hosId);
				ahds.setBizName(bizName);
				ahds.doProcessOnce(hosId);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public RESTResultBean doReceiveCheck(HttpServletRequest request, @PathVariable String hosId, @PathVariable String bizName, String jsonData) {
		...
		// 同步处理结果
		rs.setResult(mqService.getObject(HospitalJobKeyService.getSyncResultKeyName(hosId,bizName), false));
		return rs;
	}
