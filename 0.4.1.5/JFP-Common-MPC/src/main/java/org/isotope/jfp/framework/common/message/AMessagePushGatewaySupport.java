...
public abstract class AMessagePushGatewaySupport extends AMessageChannelServiceThreadSupport implements ISPushConstant {
					...
					if (doReceive()) {
						if (doInit()) {
							if (doCheck()) {
								if (doProcess()) {
									if (doFinished()) {
										logger.info(">>>>>AMessagePushGatewaySupport服务处理成功<<<<<");
									} else {
										logger.info(">>>>>AMessagePushGatewaySupport服务处理失败.....doFinished");
									}
								} else {
									logger.info(">>>>>AMessagePushGatewaySupport服务处理失败.....doProcess");
								}
							} else {
								logger.info(">>>>>AMessagePushGatewaySupport服务处理失败.....doCheck");
							}
						} else {
							logger.info(">>>>>AMessagePushGatewaySupport服务处理失败.....doInit");
						}
					} else {
						logger.info(">>>>>AMessagePushGatewaySupport服务处理失败.....doReceive");
					}
				} catch (Exception e) {
					e.printStackTrace();
	...
	public boolean doProcess() throws Exception {
		...
		logger.error(">>>>>AMessagePushGatewaySupport信息推送服务处理失败.....push......" + result);
		return false;
	}

	@Override
	public boolean doFinished() throws Exception {
		logger.debug("AMessagePushGatewaySupport数据推送成功 ......");
		return true;
	}
}
