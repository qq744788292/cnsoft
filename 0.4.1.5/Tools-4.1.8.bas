...
On Error GoTo Open_Error
 ...
 '   Print #1, ""
    ...
    Print #1, "        UPDATE " & LCase(sdbname) & " SET update_time = #{updateTime} <!-- 更新时间、更新者 -->"
    For i = dataStartLine To dataEndLine    '循环处理
        If Not getEntityValue(i) Then
            Exit For
        End If
    ...
    Print #1, "        UPDATE " & LCase(sdbname) & " SET del_flag = '1', update_time = #{updateTime}, updator = #{updator} WHERE update_time = #{updateTime}"
