...
<html>
<head>
    <title>user-role</title><!-- 角色关联用户列表 -->
    <!-- 静态引入 -->
    <c:import url="/WEB-INF/views/config/include.jsp" charEncoding="UTF-8"/>
</head>
<body>
<table>
    <form method="post" action="/90302020?token=${token}" >
        <input type="hidden" name="groupId" value="${data.puk}" />
        <input type="hidden" name="groupName" value="${data.roleName}" />
            ...
            <td>
                <table>
                    <tr class="trparam">
                        ...
                        <td class="tdparam">角色名称:${data.roleName}</td>
                        <td></td>
                        <td class="tdbtn">
...
<script type="text/javascript">
    parent.changeNavbar('权限管理', '首页》权限管理》角色定义》用户关联角色');

    var size=0;

    $.each(${userRole},function (index,item) {
