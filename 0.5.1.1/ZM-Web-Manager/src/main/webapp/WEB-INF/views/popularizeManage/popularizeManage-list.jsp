...
<html>
<head>
    <title>popularizeManage-list</title><!-- 推广员列表 -->
    <!-- 静态引入 -->
    <c:import url="/WEB-INF/views/config/include.jsp" charEncoding="UTF-8"/>
</head>
<body>
<form action="/40101010?token=${token}" method="post">
        ...
        <tr class="trparam" >
            <td>
                <table>
                    <tr>
                        <td>
                            <input type="button" style="width: 120px" onclick="parent.showPageForm('/40101020')" class="btn btn-default btnleft" value="新建" />
                        </td>
                ...
                <table class="table table-striped table-hover table-bordered">
                    ...
                    <c:forEach var="item" items="${page.pageListData}">
                        <tr>
                            ...
                            <td>
                                <c:choose>
                                    <c:when test="${item.invalidFlag == '0'}">启用</c:when>
                                    <c:when test="${item.invalidFlag == '1'}">停用</c:when>
                                    <c:otherwise>未知</c:otherwise>
                                </c:choose>
                            </td>
                            <td><input type="button" class="btn" onclick="spread_code('${item.puk}')" value="查看"></td>
                            <td class="">
                                <input type="button" class="btn btn-info" onclick="parent.showPageForm('40101030?puk=${item.puk}')" value="编辑">
                                <c:choose>
                                    <c:when test="${item.invalidFlag == '0'}">
                                        <input type="button" class="btn btn-danger"  onclick="parent.showPageForm('40101050?puk=${item.puk}&invalidFlag=1')" value="停用">
                                    </c:when>
                                    <c:when test="${item.invalidFlag == '1'}">
                                        <input type="button" class="btn btn-danger"  onclick="parent.showPageForm('40101050?puk=${item.puk}&invalidFlag=0')" value="启用">
                                    </c:when>
                                </c:choose>
                                <input type="button" class="btn btn-danger"  onclick="parent.showPageForm('40101040?puk=${item.puk}&updateTime=${item.updateTime}')" value="删除">
                            </td>
                        </tr>
                    </c:forEach>
                    </tbody>
...
<script type="text/javascript">
    parent.changeNavbar('推广管理', '首页》推广管理》推广员信息》推广员列表');
