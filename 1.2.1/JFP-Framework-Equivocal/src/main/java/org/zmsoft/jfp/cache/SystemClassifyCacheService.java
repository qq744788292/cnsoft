package org.zmsoft.jfp.cache;

import java.util.List;

import org.zmsoft.jfp.framework.support.MyDataCacheSupport;
import org.zmsoft.jfp.framework.utils.BeanFactoryHelper;
import org.zmsoft.jfp.framework.utils.EmptyHelper;
import org.zmsoft.jfp.persistent.common.SystemClassify.SystemClassifyDBO;
import org.zmsoft.jfp.persistent.common.SystemClassify.SystemClassifyService;

/**
 * 业务分类缓存
 * 
 * @author ZmSoft
 * @version 0.1.0 2018/2/8
 * @since 0.1.0 2018/2/8
 */
public class SystemClassifyCacheService extends MyDataCacheSupport<SystemClassifyDBO> {

	/**
	 * 获取一个分类的配置信息
	 *
	 * @param gameId
	 * @return
	 */
	public List<SystemClassifyDBO> loadCacheDatas() {
		String ck = CACHE_COMMON_CLASSIFY + 0;
		List<SystemClassifyDBO> configDatas = loadCacheArray(ck, SystemClassifyDBO.class);
		if (EmptyHelper.isEmpty(configDatas)) {
			SystemClassifyDBO param = new SystemClassifyDBO();
			configDatas = getDataFromDB(param);
			// 添加到缓存
			putCache(ck, configDatas);

			// 独立缓存
			for (SystemClassifyDBO item : configDatas) {
				putCache(CACHE_COMMON_CLASSIFY + item.getClassifyId(), item);
			}
		}
		return configDatas;
	}

	/**
	 * 获取配置信息
	 * 
	 * @return
	 */	
	public SystemClassifyDBO loadCacheData(String classifyId) {
		SystemClassifyDBO param = new SystemClassifyDBO();
		param.setClassifyId(classifyId);
		return loadCacheData(param);
	}

	/////////////////////////////////////////////////////////////////////////////////////////////////////////////
	public SystemClassifyDBO loadCacheData(SystemClassifyDBO param) {
		String ck = CACHE_COMMON_CLASSIFY + param.getClassifyId();
		SystemClassifyDBO configData = loadCacheClass(ck, SystemClassifyDBO.class);
		if (EmptyHelper.isEmpty(configData)) {
			List<SystemClassifyDBO> list = getDataFromDB(param);
			if (EmptyHelper.isNotEmpty(list)) {
				configData = list.get(0);
				putCache(ck, configData);
			}
		}
		return configData;
	}

	public List<SystemClassifyDBO> loadCacheDatas(SystemClassifyDBO param) {
		return loadCacheDatas();
	}

	public void removeCacheData(SystemClassifyDBO param) {
		myCacheService.removeKey(CACHE_COMMON_CLASSIFY + 0);
		myCacheService.removeKey(CACHE_COMMON_CLASSIFY + param.getClassifyId());

	}

	/**
	 * 从数据库里读取
	 * 
	 * @param type
	 * @return
	 */
	public List<SystemClassifyDBO> getDataFromDB(SystemClassifyDBO param) {
		SystemClassifyService service = BeanFactoryHelper.getBean("SystemClassifyService");
		List<SystemClassifyDBO> systemDatas = service.doSelectData(param);
		logger.debug("=====>>>>>"+systemDatas);
		return systemDatas;
	}
	
	//////////////////////////////////////
	public void loadDefaultCacheData() {
		loadCacheData(EMPTY);
	}
}
