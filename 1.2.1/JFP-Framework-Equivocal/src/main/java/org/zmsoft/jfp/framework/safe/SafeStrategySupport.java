...
import org.zmsoft.jfp.framework.support.MyRequestFailSupport;
...
public class SafeStrategySupport extends MyInterceptorAdapterSupport {
		...
		if (handler instanceof HandlerMethod) {// 判断资源路径
			...
			if (controller != null) {
				if (controller.doCheckLogin(request, response) == false) {
					MyRequestFailSupport.doTokenCheckFail(request, response, _returnType_);
					handle = false;
				}
			}
		}
	 ...
	 * 提示信息
	 */
	public final static String FIREWALL_FAIL_MESSAGE = "FIREWALL:MESSAGE";
	...
	public boolean doFireWall(String returnType, Jedis fireWallCache, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		if (min % 3 == 0 && sec < 15) {
			// 提示信息
			String msg = (String) fireWallCache.get(FIREWALL_FAIL_MESSAGE);
		...
		// 检查黑白名单
		{
			...
			if (EmptyHelper.isNotEmpty(dangerIp)) {
				MyRequestFailSupport.doFail(request, response, message);
				return false;
			}
