 ...
 */
public class MyRequestFailSupport implements IFrameworkConstants {
	/**
	 * 异常返回
	 * 
	 * @param request
	 ...
	 * @param json
	 */
	public static void doLoginFail(HttpServletRequest request, HttpServletResponse response, boolean json) {
		try {
			// JSON拦截器
			if (json) {
				RESTResultBean<String> rs = new RESTResultBean<String>();
				rs.setResult(NINE, MESSAGE_LOGIN_FAIL);
				OutputStream out = response.getOutputStream();
				out.write(JSON.toJSONString(rs).getBytes(SYSTEM_CHARSET));
				out.flush();
				out.close();
			}
			// 标准拦截器
	...
	final static String ContentType = "text/html;charset=UTF-8";

	/*
	 * 页面返回模式<br> 0：page页面直接返回<br> 1:JSON数据拦截返回
	 */
	public static void doTokenCheckFail(HttpServletRequest request, HttpServletResponse response, String returnType) {
		try {
			// 销毁session
			...
			if (ZERO.equals(returnType)) {
				...
				response.getOutputStream().write(JSON.toJSONString(result).getBytes(SYSTEM_CHARSET));
			}
		} catch (Throwable e) {
		}
	}
	 ...
	 * @param response
	 */
	public static void doFail(HttpServletRequest request, HttpServletResponse response, String message) {
		try {
			response.setCharacterEncoding(SYSTEM_CHARSET);
			response.setContentType(ContentType);
