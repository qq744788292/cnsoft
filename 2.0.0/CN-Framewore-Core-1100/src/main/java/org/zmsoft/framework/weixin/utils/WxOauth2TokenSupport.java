...
public class WxOauth2TokenSupport extends MyBusinessLogicSupport {
    ...
    */
	public JSONObject loadAccessToken(WxUserBean user) throws Exception {
		return JSON.parseObject(HttpServiceHelper.doHttpGET(String.format(ServerURL, user.getAppid(), user.getSecret(), user.getCode())));
	}
    ...
    */
	public JSONObject loadWeixinUserInfo(WxUserBean user) throws Exception {
		return JSON.parseObject(HttpServiceHelper.doHttpGET(String.format(ServerURL_GetUserInfo, user.getAccess_token(), user.getOpenid())));
	}
}
