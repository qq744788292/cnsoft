...
public class MySafeStrategyConfigurer extends MyInterceptorAdapterSupport {
	...
	MyRoleConfigSupport myRoleConfigSupport;

	public MyRoleConfigSupport getMyRoleConfigSupport() {
		if (EmptyHelper.isEmpty(myRoleConfigSupport))
			myRoleConfigSupport = MyBeanFactoryHelper.getBean(MyRoleConfigSupport.class.getSimpleName());
		return myRoleConfigSupport;
	}
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if (handler instanceof HandlerMethod) {// 判断资源路径
			...
			if (controller != null) {
				...
				//操作权限检查
				if ((handle == true) && (getMyRoleConfigSupport().isRoleCheck() == true) && (controller.doCheckRole(request, response) == false)) {
