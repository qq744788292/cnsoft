...
public class MySystemConfigParameterSupport extends MyServiceSupport {
	...
	public KeyValueBean loadCacheData(int classify, String type, String key) throws Exception {
		...
		if (classify == 1) {
			SystemConfigDBO config = mySystemConfigCache.loadConfig(key);
			kvb.setKey(config.getKey());
			kvb.setValue(config.getValue());
		} else if (classify == 2) {
			SystemParameterDBO parameter = mySystemParameterCache.loadParameter(type, key);
	...
	public Map<String,String> loadCacheDatas(int classify, String type) throws Exception {
		...
		if (classify == 1) {
			for (SystemConfigDBO item : mySystemConfigCache.loadConfigs(true)) {
				map.put(item.getKey(),item.getValue());
			}
		} else if (classify == 2) {
			for (SystemParameterDBO item : mySystemParameterCache.loadParameters(type)) {
				map.put(item.getKey(),item.getValue());
			}
		}
