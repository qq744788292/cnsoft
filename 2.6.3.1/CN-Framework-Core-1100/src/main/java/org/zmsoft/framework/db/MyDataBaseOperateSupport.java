 ...
 */
public class MyDataBaseOperateSupport<T> implements ISDataSourceName, ICFrameworkConstants, ICDBConstants {
	...
	@Transactional(rollbackFor = { Exception.class, RuntimeException.class })
	public int doInsertOrUpdate(MyDataBaseDefaultSupportBean formParamBean, int intercept, boolean simpleCheck, boolean selective) {
	 ...
	 */
	@Transactional(rollbackFor = { Exception.class, RuntimeException.class })
	public int[] doDetailInsert(MyDataBaseDefaultSupportBean formParamBean, MyDataBaseDefaultSupportBean[] datas, boolean selective) {
		if (EmptyHelper.isEmpty(datas))
			return new int[0];
		// 根据外键删除所有数据
	...
	@Transactional(rollbackFor = { Exception.class, RuntimeException.class })
	public int[] doInsert(MyDataBaseDefaultSupportBean[] datas, boolean selective) {
		if (EmptyHelper.isEmpty(datas))
			return new int[0];
		int[] rs = new int[datas.length];
		for (int i = 0; i < rs.length; i++) {
	...
	@Transactional(rollbackFor = { Exception.class, RuntimeException.class })
	public int[] doInsert(List<? extends MyDataBaseDefaultSupportBean> datas, int intercept, boolean selective) {
		if (EmptyHelper.isEmpty(datas))
			return new int[0];
		int[] rs = new int[datas.size()];
		for (int i = 0; i < rs.length; i++) {
	 ...
	 * @return
	 */
	public int doInsertSelective(MyDataBaseDefaultSupportBean formParamBean) {
	 ...
	 * @return
	 */
	public int doInsert(MyDataBaseDefaultSupportBean formParamBean, int intercept, boolean selective) {
		// 主键补全
	...
	public int doUpdate(MyDataBaseDefaultSupportBean formParamBean, int intercept) {
		return doUpdate(formParamBean, 2, dataLock);
	}

	public int doUpdate(MyDataBaseDefaultSupportBean formParamBean, int intercept, boolean lock) {
		// 更新者、更新时间
	...
	public void doUpdateAll(MyDataBaseDefaultSupportBean formParamBean, int intercept) {
		doUpdateAll(formParamBean, 2, dataLock);
	}

	public void doUpdateAll(MyDataBaseDefaultSupportBean formParamBean, int intercept, boolean lock) {
		// 更新者、更新时间
		formParamBean.prepareUpdator(intercept, lock);
