...
@CrossOrigin
@RestController
@RequestMapping(value = "/common/1.0/token", method = { RequestMethod.POST })
public class ApiTokenCheckController extends MyTokenCommonSupport {
	...
	public RESTResultBean<UserBean> userInfo(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String token = super.loadToken(request, response);
		String jobId = super.loadJobId(request, response);
		result.setCode(1);// 不是
		if (doCheckToken(request, jobId, token) && doCheckGuest(request, jobId, token))
