...
public class WeixinOrderCheckService extends MyOrderServiceSupport {
	...
	public boolean doUserOrderCheck(WxPayCallBackBean weiXinCallBackBean) throws Exception {
		try {
			...
			_WxOrderCheckBean_.setKey(getWxPayConfigService().getWxCustomSecret());
			_WxOrderCheckBean_.setAppid(getWxPayConfigService().getWxAppID());// 应用ID
			_WxOrderCheckBean_.setMch_id(getWxPayConfigService().getWxCustomid());// 商户号
