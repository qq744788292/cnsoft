...
import org.zmsoft.config.wx.WxMenuConfigService;
 ...
 */
@Controller
public class WxHomeController extends MyWeixinCommonSupport {
	
	@Resource
	WxMenuConfigService wxMenuConfig;
	 ...
	 * @since 0.1.0 2018/4/2
	 */
	@RequestMapping(value = "/WXHome/{menuId}", method = RequestMethod.GET)
	public ModelAndView doWXHomeIndex(@PathVariable String menuId) throws Exception {
		...
		// 回调地址
		model.addObject("redirect", URLEncoder.encode(getSystemConfigService().getWxServerUrl() + "/WXHome/Back/" + menuId, SYSTEM_CHARSET));
		// 目标地址
	 ...
	 * @since 0.1.0 2018/4/2
	 */
	@RequestMapping(value = "/WXHome/Back/{menuId}", method = RequestMethod.GET)
	 ...
	 * @since 0.1.0 2018/4/2
	 */
	@RequestMapping(value = "/WXHome/login/{menuId}", method = RequestMethod.GET)
	public ModelAndView doWXLogin(HttpServletRequest request, HttpServletResponse response, @PathVariable String menuId, WxCallBackParamBean wxCallBackParamBean) throws Exception {
		...
		model.addObject("menuId", menuId);
		model.addObject("menuUrl", wxMenuConfig.getMenu(menuId));
