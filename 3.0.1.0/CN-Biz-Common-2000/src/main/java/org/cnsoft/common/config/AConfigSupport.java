...
public abstract class AConfigSupport extends ObjectBean implements ICFrameworkConstants {
	...
	public void setReload(boolean reload) {
		// 初始化
		loadMySystemConfigParameterSupport().setReload(reload);
	}
	...
	public void init() throws Exception {
		...
		// 加载系统配置SystemConfig s901010_config
		//prepareSystemConfigByclass(this);
		// 加载参数配置SystemParameter s901060_parameter
	...
	public void prepareSystemConfigByclass(AConfigSupport clazz) throws Exception {
		if (EmptyHelper.isEmpty(loadMySystemConfigParameterSupport())) {
			return;
		}
		// 获取所有参数
		prepareFieldValue(loadMySystemConfigParameterSupport().loadCacheDatas(1, EMPTY), clazz);
	}
	...
	public void prepareSystemParameterByclass(String type, AConfigSupport clazz) throws Exception {
		...
		// 获取所有参数
		prepareFieldValue(loadMySystemConfigParameterSupport().loadCacheDatas(2, type), clazz);
	}
	...
	public void prepareFieldValue(Map<String, String> configs, AConfigSupport clazz) throws Exception {
		...
		for (Field field : fields) {
			try {
				// 获取原来的访问控制权限
				boolean accessFlag = field.canAccess(clazz);
				if (accessFlag == false)
