...
import org.cnsoft.framework.job.bean.JobLogBean;
 ...
 */
public abstract class ADynamicTaskJob implements Job, ICFrameworkConstants {
	...
	protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
		...
		try {
			...
			if ("1000".equals(scheduleJobBean.getJobType())) {
			...
			} else if ("2000".equals(scheduleJobBean.getJobType())) {
				...
				if ("POST".equals(scheduleJobBean.getApiMode().toUpperCase())) {
					...
					if (EmptyHelper.isNotEmpty(scheduleJobBean.getApiParam())) {
						param = JSONObject.parseMap(scheduleJobBean.getApiParam());
					}
					// 请求接口
	...
	public void saveJobLog(JobLogBean jobLogBean) {
		...
		// _ELKLogSendConnection_.sendLogData(LOG_TASK_KEY, jobLogBean.toJsonString());
	}
}
