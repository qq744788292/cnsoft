Attribute VB_Name = "ZMSoft_Tools_3000"
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
'数据库与CN-Soft(原JFP)框架代码片段生成工具
'
'版本 3.0.0.0   2021-03-19
'
'作者 ZM-Soft
'
'日志说明
'
'   3.0.0.0   2021-03-19 配合3.0.0版本框架升级发布
'   2.6.3.1   2020-01-19 优化持久层XML里面doUpdate方法的参数if判断，去掉空白判定
'   2.6.2.6   2019-12-20 增加doInsertSelective方法，优化insert逻辑，采取非空默认值模式
'   2.6.2.3   2019-10-12 优化List页面逻辑，允许输出扩展字段，优化输出路径
'   2.6.2.1   2019-09-29 优化JSP输出页面逻辑，允许输出memo字段
...
Sub CreatAll()
    ...
    For i = 4 To ActiveWorkbook.Worksheets.Count
        ...
        sdbname = Cells(1, dbNameCol).Value
        If sdbname = "" Then
            Exit For
        End If
...
Function getSQLValue(line As Integer) As Boolean
    ...
    memoValue = Trim(Cells(line, memoCol).Value)
...
Sub targetFileName()
    ...
    sdbname = SYSNAME & Cells(1, dbNameCol).Value
...
On Error GoTo Open_Error
    ...
    '插入信息
    WriteStream.WriteText "    <!-- 插入一条数据（使用空值） -->" & vbCrLf
    ...
    Dim selectiveCol         As String
    Dim selectiveVal         As String
    
    WriteStream.WriteText "    <!-- 插入一条数据（使用默认值） -->" & vbCrLf
    WriteStream.WriteText "    <insert id='doInsertSelective' parameterType=" & sTemp & ">" & vbCrLf
    WriteStream.WriteText "        INSERT INTO " & LCase(sdbname) & vbCrLf
    ...
    For i = dataStartLine To dataEndLine     '循环处理
        ...
        '字段部分序列-----------------------------------------
        If propertiesValue <> "String" Then
            selectiveCol = selectiveCol + "            <if test=' " & fieldULValue & " != null '><!-- " & itemValue & " -->" & vbCrLf
        Else
            selectiveCol = selectiveCol + "            <if test=' " & fieldULValue & " != null and " & fieldULValue & " !="""" '><!-- " & itemValue & " -->" & vbCrLf
        End If
            
        If i <> dataStartLine Then
            selectiveCol = selectiveCol + "                , " & LCase(fieldValue) & vbCrLf
        Else
            selectiveCol = selectiveCol + "                 " & LCase(fieldValue) & vbCrLf
        End If
        
        selectiveCol = selectiveCol + "            </if>" & vbCrLf
            
            
        '内容部分序列-----------------------------------------
        If propertiesValue <> "String" Then
            selectiveVal = selectiveVal + "            <if test=' " & fieldULValue & " != null '><!-- " & itemValue & " -->" & vbCrLf
        Else
            selectiveVal = selectiveVal + "            <if test=' " & fieldULValue & " != null and " & fieldULValue & " !="""" '><!-- " & itemValue & " -->" & vbCrLf
        End If
            
        If i <> dataStartLine Then
            selectiveVal = selectiveVal + "                , " & " #{" & fieldULValue & "}" & vbCrLf
        Else
            selectiveVal = selectiveVal + "                 " & " #{" & fieldULValue & "}" & vbCrLf
        End If
        
        selectiveVal = selectiveVal + "            </if>" & vbCrLf
        
    Next i

    WriteStream.WriteText "        (" & vbCrLf
    WriteStream.WriteText selectiveCol
    WriteStream.WriteText "        )" & vbCrLf
    
    WriteStream.WriteText "        VALUES " & vbCrLf
    
    WriteStream.WriteText "        (" & vbCrLf
    WriteStream.WriteText selectiveVal
    WriteStream.WriteText "        )" & vbCrLf
    WriteStream.WriteText "    </insert>" & vbCrLf
    ...
    WriteStream.WriteText "            <if test=' updateTime != null '><!-- 更新时间 -->" & vbCrLf
...
On Error GoTo Open_Error
    ...
    WriteStream.WriteText " */" & vbCrLf
    WriteStream.WriteText "public class " & upFirstStr(ULCase(sdbname)) & "DBO extends MyDataBaseObjectSupport2" & vbCrLf
...
On Error GoTo Open_Error
    ...
    WriteStream.WriteText "        // 排序模式" & vbCrLf
    WriteStream.WriteText "        //pageModel.setOrderbyCreateTimeDESC();" & vbCrLf
...
On Error GoTo Open_Error
    ...
    ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    'Open sfileName & "Controller.java" For Output Shared As #1
    sfileName = SelectPath + "model"
    If Dir(sfileName, vbDirectory) = "" Then
        MkDir sfileName
    End If
    soutfileName = SelectPath + "model\" + FolderPath & "PageController.java"
    ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    Dim WriteStream As Object
    Set WriteStream = CreateObject("ADODB.Stream") '文件流输出模式
    WriteStream.Type = 2     'adTypeText
    ...
    WriteStream.WriteText "public class " & upFirstStr(ULCase(sdbname)) & "PageController extends MyPageControllerSupport { //MyTokenCommonSupport" & vbCrLf
    ...
    WriteStream.WriteText "            " & daoName & ".doInsertSelective(param);" & vbCrLf
...
On Error GoTo Open_Error
    ...
    sfileName = SelectPath + "jsp"
    If Dir(sfileName, vbDirectory) = "" Then
        MkDir sfileName
    End If
    soutfileName = SelectPath + "jsp\" + FolderPath + "-item.jsp"
    ...
    WriteStream.WriteText "<meta name=""description"" content=""ZM version.1.6.2 since." + Format(Now, "yyyymmddHHmmss") + """>" & vbCrLf
    ...
    WriteStream.WriteText "<script src=""/custom/views/js/xxx/" + sdbname + "-page.js""></script>" & vbCrLf
...
On Error GoTo Open_Error
    ...
    If Dir(sfileName, vbDirectory) = "" Then
        MkDir sfileName
    End If
    soutfileName = SelectPath + "jsp\" + FolderPath + "-list.jsp"
    sdbname = LCase(ULCase(sdbname))
    SYSNAME = "/page/1.0/base/" + sdbname
    ...
    WriteStream.WriteText "<script src=""/custom/views/js/xxx/" + sdbname + "-list.js""></script>" & vbCrLf
    WriteStream.WriteText "</body>" & vbCrLf
    WriteStream.WriteText "</html>" & vbCrLf
    WriteStream.WriteText "" & vbCrLf
...
On Error GoTo Open_Error
    ...
    WriteStream.WriteText "import org.springframework.stereotype.Component;" & vbCrLf
    WriteStream.WriteText "" & vbCrLf
    WriteStream.WriteText "/**" & vbCrLf
    ...
    WriteStream.WriteText "public class " & upFirstStr(ULCase(sdbname)) & "Businesslogic extends MyBusinessSupport {" & vbCrLf
    ''--------------------------------------------------------------------------------------------------
    WriteStream.WriteText "    @Resource" & vbCrLf
    WriteStream.WriteText "    private " & upFirstStr(ULCase(sdbname)) & "Dao " & daoName & ";" & vbCrLf
    WriteStream.WriteText "" & vbCrLf
...
On Error GoTo Open_Error
    ...
    WriteStream.WriteText "    public HashMap<String, Object> loadApiDocValue() throws Exception {" & vbCrLf


    ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
    WriteStream.WriteText "        HashMap<String, Object> all = new HashMap<String, Object>();" & vbCrLf
    ...
    WriteStream.WriteText "        {" & vbCrLf
    WriteStream.WriteText "            ArrayList<Object> list = new ArrayList<Object>();" & vbCrLf
    ...
    WriteStream.WriteText "        {" & vbCrLf
    WriteStream.WriteText "            ArrayList<Object> list = new ArrayList<Object>();" & vbCrLf
    WriteStream.WriteText "            HashMap<String, Object> item;" & vbCrLf
    '循环处理
    'set
    Dim i_start           As Integer
    i_start = dataStartLine
    ...
    For i = i_start To dataEndLine
        ...
        If (LCase(fieldValue) <> PRI_KEY) Then
            '/**********项目设置**********/
            WriteStream.WriteText "            {" & vbCrLf
            WriteStream.WriteText "                item = new HashMap<String, Object>();" & vbCrLf
            ...
            WriteStream.WriteText "                item.put(""memo"", """ + memoValue + """);" & vbCrLf
            WriteStream.WriteText "                " & vbCrLf
