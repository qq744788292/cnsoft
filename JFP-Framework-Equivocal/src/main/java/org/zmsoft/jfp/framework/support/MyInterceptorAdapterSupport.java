...
public class MyInterceptorAdapterSupport extends HandlerInterceptorAdapter implements IFrameworkConstants{
	 ...
	 * @param json
	 */
	public void doFail(HttpServletRequest request, HttpServletResponse response, boolean json) {
		try {
			// JSON拦截器
			if (json) {
				RESTResultBean<String> rs = new RESTResultBean<String>();
				rs.setCode("9");
				rs.setMsg("9");// 登录失效，请重新登录
	 ...
	 * @param response
	 */	
	public void doFail(HttpServletRequest request, HttpServletResponse response,String message) {
		try {
